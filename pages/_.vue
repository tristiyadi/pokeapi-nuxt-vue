			...
			<div class="z-0 max-w-screen-2xl flex flex-col h-screen p-5 pt-56 w-screen">
				<div v-if="pokemon" class="no-scrollbar text-gray-700 dark:text-white overflow-auto flex-grow flex flex-col min-w-full space-y-5">
					<div class="flex flex-col items-center justify-center mt-14">
						<span class="break-words text-justify text-sm">Description: {{ pokemonDescription() }}</span>
					</div>
					<pokemon-type-badge
						:types="pokemon ? pokemon.types : []"
						:full="true"
					/>
					<span class="font-black pt-2 pb-2 text-xl" :class="`text-${pokemonColor()}`">Pokemon Data</span>
					<div class="grid grid-cols-2 gap-4 mt-2 font-black text-xs">
						<div class="grid grid-cols-2">
							<span class="col-span-1 text-gray-700 dark:text-white text-xl">Height</span>
							<span class="col-span-1 text-xl" :class="`text-${pokemonColor()}`">{{ pokemon.height / 10 }} m</span>
						</div>
						<div class="grid grid-cols-2">
							<span class="col-span-1 text-gray-700 dark:text-white text-xl">Weight</span>
							<span class="col-span-1 text-xl" :class="`text-${pokemonColor()}`">{{ pokemon.weight / 10 }} kg</span>
						</div>
					</div>
					...
					<div v-else class="grid flex-grow">
						<span class="justify-self-center self-center" :class="`text-${pokemonColor()}`">This Pokemon does not evolve.</span>
					</div>
