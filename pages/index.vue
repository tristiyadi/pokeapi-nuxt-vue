<template>
	<div class="flex flex-col h-screen max-w-screen-2xl min-h-screen mx-auto p-5">
		...
		<header class="bg-transparent">
			<form class="py-3 space-y-4" autocomplete="off" @submit.prevent="onSearchSubmit">
				<div class="grid grid-cols-12">
					<h1 class="col-span-11 text-red-600 text-3xl font-semibold">Pokeapi</h1>
					<svg
						fill="none"
        ...
        <div class="grid grid-cols-12" style="display:flex;flex-direction:row;">
          <div class=" space-x-6">
            <svg
              v-if="isSearching"
              width="22"
          ...
          <!-- <div class=" space-x-6">
            <select id="select-pokemon" ref="select-pokemon" v-model="selectKey">
              <option  v-for="(type, index) in pokemonType" :key="index" v-bind:value="type" @change="myChangeEvent" @select="mySelectEvent" >
                  {{ type }}
              </option>
            </select>
          </div> -->
        </div>
				<span class="block md:pt-4 lg:pt-4 font-medium text-xs text-gray-500 dark:text-white">The Pokeapi contains detailed stats for every creature from the Pokemon games.</span>
			</form>
		</header>
        ...
        <button style="color:rgb(50 204 219);float:right;" class="h-full m-auto cursor-pointer btn btn-primary" v-if="prevUrl" @click="prevPage()">Prev</button>
			</div>
			<div class="h-full w-full grid-cols-1 justify-self-center self-center">
        <button style="color:rgb(50 204 219)" class="h-full m-auto cursor-pointer btn btn-primary" v-if="nextUrl" @click="nextPage()">Next</button>
			</div>
...
export default class IndexPage extends mixins(ChangeTheme) {
  ...
  get pokemonType(){
    const pokemonType = this.$accessor.pokemon.listTypeResponse.results;
    ...
    console.log(this.$route)
		...
		if (search && search.length >= 3) {
			...
			this.$accessor.pokemon.getListPokemonType();
		} else {
			this.$router.push('/');
  ...
  mySelectEvent(){
    this.$accessor.pokemon.getPokemonType(this.selectKey);
  }
  myChangeEvent(){
    this.$accessor.pokemon.getPokemonType(this.selectKey);
  }
	...
	activated() {
		...
		this.$accessor.pokemon.setListResponseType(this.$accessor.pokemon.listTypeResponse);
	}
