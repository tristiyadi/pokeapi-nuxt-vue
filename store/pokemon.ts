...
export const state = () => ({
	...
	pokemonListType: [] as PokemonList[],
	pokemonSpecies: [] as PokemonSpecies[],
	listResponse: {} as PokemonListResponse,
	listTypeResponse: {} as PokemonListResponse,
...
export const mutations = mutationTree(state, {
  ...
  SET_LIST_RESPONSE_TYPE(state, listTypeResponse: PokemonListResponse) {
		state.listTypeResponse = listTypeResponse;
	},
...
export const actions = actionTree({ state, getters, mutations }, {
  ...
  setListResponseType({ commit }, listTypeResponse: PokemonListResponse) {
		commit('SET_LIST_RESPONSE_TYPE', listTypeResponse);
	},
  ...
  async getPokemonAllType({ commit }) {
		try {
			const response = await this.$axios({
				method: 'get',
				url: `${ENDPOINTS.POKEMON_TYPE}`,
			});
			if (response.status === 404) {
				return;
			}
  ...
  async getListPokemonType({ commit }) {
		let result;
		try {
			const response = await this.$axios({
				method: 'get',
      ...
      const results = humps(response.data) as PokemonType;
			const pokemonSearch: PokemonList[]=[];
      results.pokemon.map( v => {
        pokemonSearch.push(v.pokemon);
      });
			result = { results: pokemonSearch, count: pokemonSearch.length, previous: '', next: '' } as PokemonListResponse;
			commit('SET_PREV_URL', '');
			commit('SET_NEXT_URL', '');
			commit('SET_LIST_RESPONSE_TYPE', result);
			return result;
		} catch (err) {
			return undefined;
		}
